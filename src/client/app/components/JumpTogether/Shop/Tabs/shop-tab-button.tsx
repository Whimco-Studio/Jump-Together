...
export interface ShopTabButtonProperties extends React.PropsWithChildren {
	active: boolean;
	index: number;
	label: string;
	onClick: () => void;
...
export default function ShopTabButton(properties: ShopTabButtonProperties): React.Element {
	...
	return (
		<frame
			...
			key={`ShopTabButton-${properties.index}`}
		>
			<imagebutton
				...
				Size={size}
				key={`ShopTabButton-${properties.index}-ImageButton`}
			>
				<textlabel
					AnchorPoint={new Vector2(0.5, 0.5)}
					BackgroundColor3={Color3.fromRGB(255, 255, 255)}
