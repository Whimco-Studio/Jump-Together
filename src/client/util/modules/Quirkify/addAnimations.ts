...
class AddAnimations {
	...
	loadAnimation(animationId: number): AnimationTrack {
		const animationIdStr = tostring(animationId);
		if (this.loadedAnimations[animationIdStr]) {
			return this.loadedAnimations[animationIdStr];
		}
		...
		this.loadedAnimations[animationIdStr] = loadedAnimation;
	...
	playAnimation(animationName: string, transitionTime?: number) {
		...
		if (this.currentAnimTrack) {
			this.currentAnimTrack.Stop(transitionTime !== undefined ? transitionTime : 0.1);
		...
		if (animationId !== undefined) {
			...
			this.currentAnimTrack.Play(transitionTime !== undefined ? transitionTime : 0.1);
